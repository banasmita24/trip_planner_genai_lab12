...
def generate_mock_flights(destination: str, start: date, days: int) -> list[dict]:
    """Return a list of mock flight options for display purposes only."""
    return [
        {
            "airline": "SkyWays",
...
def generate_mock_hotels(destination: str, start: date, days: int) -> list[dict]:
    """Return a list of mock hotel options for display purposes only."""
    return [
        {
...
def main() -> None:
    ...
    if st.button("Plan my trip"):
        ...
        with st.spinner("Fetching weather information..."):
            ...
            if not ow_api_key:
                st.warning(
                    "OpenWeather API key is not configured; weather lookup is disabled."
                )
                weather_data = None
            else:
        ...
        with st.spinner("Asking Groq to craft your trip..."):
            llm = get_llm()
            if not llm:
                return
